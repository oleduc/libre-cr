...
pub fn presentation_tool_schemas() -> Vec<ToolSchema> {
    vec![
        ToolSchema {
            ...
            description: "Highlight a range of lines in the PR diff shown in the browser. Use this whenever the reviewer asks you to point out, show, mark or highlight code. ALWAYS pass `label` (the short heading of the part, ≤ 8 words — shown as a caption next to the code) AND `detail` (your explanation of that part, 1–3 plain sentences — shown in the reviewer's tour widget next to the highlight, so it must stand on its own without the chat). Line numbers are NEW-side (right/after) numbers of the PR head; for deleted lines use the OLD-side number. Works for any file in the PR diff (the browser scrolls the file into view if needed). Keep each range tight — the specific lines you are describing (a signature, a condition, a short block), at most 80 lines. To point at a large class or a whole module, make several narrow highlights at the lines that matter instead of one sweeping range; a longer range is clamped to its first 80 lines.".into(),
